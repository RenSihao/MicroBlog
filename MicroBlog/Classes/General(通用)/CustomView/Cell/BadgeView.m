...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        ...
        self.font = [UIFont fontWithName:kDefaultRegularFontFamilyName size:13.0];
        self.point = NO;
...
- (CGPathRef)newBadgePathForTextSize:(CGSize)inSize
{
    ...
    CGFloat arcRadius = ceil((inSize.height + expand) / 2.2f);
...
- (void)setValue:(NSString *)value
{
    if(_value != value)
    {
        ...
        {
//            _value = [value copy];
            _value = [[NSString stringWithFormat:@"N"] copy];
            self.hidden = self.hiddenWhenZero;
        }
