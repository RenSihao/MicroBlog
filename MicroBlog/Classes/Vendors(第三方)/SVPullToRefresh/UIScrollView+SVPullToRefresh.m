    ...
    if(!self.pullToRefreshView) {
        ...
        switch (position) {
            case SVPullToRefreshPositionTop:
                yOrigin = - ( SVPullToRefreshViewHeight + originalTopInset );
                break;
...
- (id)initWithFrame:(CGRect)frame {
    if(self = [super initWithFrame:frame]) {
        ...
        self.textColor = [UIColor colorWithWhite:184/255.0 alpha:1.0];
...
- (void)layoutSubviews {
    ...
    if(hasCustomView) {
        ...
        if ([[UIDevice currentDevice].systemVersion floatValue]>=7) {
            ...
            CTFontRef ctfontForTitle = CTFontCreateWithName((CFStringRef)self.titleLabel.font.fontName, self.titleLabel.font.pointSize, NULL);
            [tmpDict setObject:(__bridge id)(ctfontForTitle) forKey:(id)kCTFontAttributeName];
            CFRelease(ctfontForTitle);
            titleSize = [self.titleLabel.text boundingRectWithSize:CGSizeMake(labelMaxWidth, self.titleLabel.font.lineHeight) options:NSStringDrawingUsesLineFragmentOrigin attributes:tmpDict context:nil].size;
            titleSize.width+=10;
        }else{
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if([keyPath isEqualToString:@"contentOffset"])
        ...
        switch (self.position) {
            case SVPullToRefreshPositionTop:
                yOrigin = - (SVPullToRefreshViewHeight + self.originalTopInset);
                break;
            case SVPullToRefreshPositionBottom:
...
//		_arrow = [[SVPullToRefreshArrow alloc]initWithFrame:CGRectMake(0, self.bounds.size.height-54, 22, 48)];
        _arrow = [[SVPullToRefreshArrow alloc]initWithFrame:CGRectMake(0, self.bounds.size.height-54, 11, 48)];
...
- (void)drawRect:(CGRect)rect {
     ...
     **/
    UIImage *arrowImg = [UIImage imageNamed:@"icon_pulltorefresh.png"];
    [arrowImg drawAtPoint:CGPointMake(CGRectGetMidX(rect)-arrowImg.size.width*0.5, CGRectGetMidY(rect) - arrowImg.size.height*0.5)];
}
